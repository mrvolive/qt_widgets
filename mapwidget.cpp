...
MapWidget::MapWidget(QWidget *parent)
    : QWidget(parent), _zoom(10), _pendingRequests(0), _isDragging(false),
...
QPoint MapWidget::lonLatToTile(double lon, double lat, int zoom) {
  ...
  int y = static_cast<int>((1.0 - log(tan(latRad) + 1.0 / cos(latRad)) / M_PI) /
                           2.0 * n);
  return QPoint(x, y);
}
...
void MapWidget::loadTiles() {
  ...
  int factor = 2; // Même facteur que dans renderFullView
...
void MapWidget::renderFullView() {
  // Créer une image 4 fois plus grande que la taille du widget
  // Au lieu d'une marge fixe de 256 pixels, nous allons utiliser un facteur multiplicatif
  ...
  for (int i = 0; i < _tiles.size(); i++) {
    ...
    // Calculer la position d'affichage de la tuile avec le décalage fractionnaire
    int halfTileSize = tileSize / 2;
    int x = centerX + (tilePos.x() - floor(centralTileF.x())) * tileSize -
            pixelOffsetX - halfTileSize;
    int y = centerY + (tilePos.y() - floor(centralTileF.y())) * tileSize -
            pixelOffsetY - halfTileSize;
